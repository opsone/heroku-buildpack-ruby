    ...
    def run!(command, options = {})
      ...
      unless $?.success?
        if max_attempts > 1
          ...
          puts "Command: '#{command}' failed, retrying #{remaining_attempts} more time#{"s" if remaining_attempts != 1}."
          return run!(command, options.merge(max_attempts: remaining_attempts))
        else
